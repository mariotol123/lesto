<?php
/**
 * Template name: Realizzazioni
 *
 * @link https://developer.wordpress.org/themes/basics/template-hierarchy/
 *
 * @package lesto-theme
 */

get_header();
?>

<!-- Main content area -->
<main>
    <div class="realizzazioni-container">
        <div class="container pt-7xl d-flex justify-content-between">
            <h2 class="">Realizzazioni</h2>
            <div class="d-flex align-items-center gap-4">
                <h5 class="active">CATEGORIE</h5>
                <h5>CLIENTI</h5>
            </div>
        </div>
        <hr class="footer-divider border-2 opacity-75 w-100">
        <!-- Due colonne sotto l'hr -->
        <div class="container mt-4 pb-10xl">
            <div class="row">
                <div class="col-md-3 d-flex flex-column gap-3">
                    <p class="desktop-p-16 active">tutte le realizzaioni</p>
                    <p class="desktop-p-16">cucine</p>
                    <p class="desktop-p-16">impianti di aspirazione</p>
                    <p class="desktop-p-16">arredo bar</p>
                    <p class="desktop-p-16">cucine aziendali</p>
                    <p class="desktop-p-16">lestowatt</p>
                </div>
                <div class="col-md-9">
                    <div class="mosaico-cards">
                        <div class="mosaico-cards__item">
                            <img src="/wp-content/themes/lesto-theme/images/clay-banks-ghN8sI7MDpM-unsplash.jpg" class="mosaico-img" alt="img1">
                            <p class="desktop-p-16">Titolo lavoro</p>
                        </div>
                        <div class="mosaico-cards__item">
                            <img src="/wp-content/themes/lesto-theme/images/dwayne-joe-FVNCHQiQCq4-unsplash.jpg" class="mosaico-img" alt="img2">
                            <p class="desktop-p-16">Titolo lavoro</p>
                        </div>
                        <div class="mosaico-cards__item">
                            <img src="/wp-content/themes/lesto-theme/images/pesce-huang-RWLTHuxCqYo-unsplash.jpg" class="mosaico-img" alt="img3">
                            <p class="desktop-p-16">Titolo lavoro</p>
                        </div>
                        <div class="mosaico-cards__item">
                            <img src="/wp-content/themes/lesto-theme/images/sindy-sussengut-SVTBVz8mcOY-unsplash.jpg" class="mosaico-img" alt="img4">
                            <p class="desktop-p-16">Titolo lavoro</p>
                        </div>
                        <div class="mosaico-cards__item">
                            <img src="/wp-content/themes/lesto-theme/images/steve-busch-_PLxJoYCUTY-unsplash.jpg" class="mosaico-img" alt="img5">
                            <p class="desktop-p-16">Titolo lavoro</p>
                        </div>
                        <div class="mosaico-cards__item">
                            <img src="/wp-content/themes/lesto-theme/images/willian-justen-de-vasconcellos-HIgLoYYY2qU-unsplash.jpg" class="mosaico-img" alt="img6">
                            <p class="desktop-p-16">Titolo lavoro</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
</main>

<?php

get_footer();
